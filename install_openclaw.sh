...
fn_6_config_sync() {
    check_env || return 1
    echo -e "${YELLOW}🔥 基地淨化：焚毀舊緩存與錯誤連線紀錄...${NC}"
    ...
    echo -e "${YELLOW}請輸入模型 (建議先按 m 查詢，如: gemini-2.5-flash):${NC}"
...
EOT
    ...
    echo -e "${YELLOW}🛡️ 正在注入強化組件 (Search, Temp, Instruction)...${NC}"
    sudo -u $REAL_USER node dist/index.js agents create --name main 2>/dev/null
    
    # 核心強化指令
    sudo -u $REAL_USER node dist/index.js config set agents.main.enabled true
    sudo -u $REAL_USER node dist/index.js config set agents.main.model "google/$SELECTED_MODEL"
    sudo -u $REAL_USER node dist/index.js config set agents.main.system_instruction "$USER_SYS"
    sudo -u $REAL_USER node dist/index.js config set agents.main.tools "['google_search_retrieval']"
    sudo -u $REAL_USER node dist/index.js config set agents.main.tools_config.google_search_retrieval.dynamic_retrieval_threshold 0.3
    sudo -u $REAL_USER node dist/index.js config set agents.main.model_config.temperature 0.1
    sudo -u $REAL_USER node dist/index.js config set agents.main.model_config.max_output_tokens 4096
...
fn_restart() { fn_stop; sleep 1; fn_start; echo -e "${GREEN}✅ GAPOON 已強化啟動。${NC}"; }
...
show_menu() {
    clear; stty sane
    echo -e "${GREEN}==========================================${NC}"
    echo -e "    🦞 OpenClaw Manager V3.7.1"
    echo -e "${GREEN}==========================================${NC}"
    echo -e " 1) 系統工具安裝"
    echo -e " 2) Node 環境建置"
    echo -e " 3) 代碼更新"
